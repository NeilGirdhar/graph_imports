...
def parse_dependencies(base_name: str,
    ...
    with redirect_stdout(string_io):
        _ = pydeps.call_pydeps(base_name, max_bacon=1, show_dot=True, no_output=True)
...
def main(base_name: str) -> None:
    ...
    _ = dot_stringio.write('\n'.join(header))
    for n in used_nodes:
        some_dict: dict[str, str] = node_mapping[n]
        some_dict['label'] = f'"{n}"'
        _ = dot_stringio.write(f'    {n} {attrs2fmt(some_dict)}')
    for (a, b), fmt_dict in rule_mapping.items():
        _ = dot_stringio.write(f'    {a} -> {b} {attrs2fmt(fmt_dict)}')
    _ = dot_stringio.write('}')
